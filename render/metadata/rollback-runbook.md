...
4. If a service worker is implicated, deploy a no-cache service worker under `/render/sw.js` that deletes `civgraph-test-*` and `civgraph-test-v*` caches, then unregister it in a follow-up once clients have updated.
...
4. Confirm `/render/` loads in a clean browser profile.
