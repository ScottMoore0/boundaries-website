...
Promotion and rollback requirements are tracked in `render/metadata/production-readiness.md`.
...
- `sourceType: "pmtiles"` is used only when `/render/pmtiles/generated/<layer-id>.pmtiles` exists and is below the configured size budget.
...
This writes `render/metadata/cdn-upload-manifest.json` with local paths, target keys, CDN URLs, and example `wrangler r2 object put` commands. Defaults are:
...
The verification script writes `render/metadata/cdn-range-report.json` and fails if any archive does not return a 16-byte `206 Partial Content` response for `Range: bytes=0-15`.
...
```powershell
git rm --cached -- render/pmtiles/generated/*.pmtiles
```
