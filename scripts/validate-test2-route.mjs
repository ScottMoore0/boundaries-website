...
assert(electionManagerSource.includes('test2ElectionCountDetail') && electionManagerSource.includes('election-detail-toggle-btn--header') && electionManagerSource.includes('Detailed View: On'), '/test2 count tables must expose the main-style detailed count toggle in the pane header');
...
assert(electionManagerSource.includes("renderOverallResults(view = 'party') {\n    return this.sharedRenderer.renderOverallResults(view);") && electionManagerSource.includes("renderConstituencyResults(result, view = 'party') {\n    return this.sharedRenderer.renderConstituencyResults(result, view);"), '/test2 visible election pane rendering must enter through the shared renderer, not bypass it with route-specific branches');
assert(electionManagerSource.includes('renderMainCompatibleOverallResults') && electionManagerSource.includes('renderMainCompatibleConstituencyResults'), '/test2 must expose main-compatible election pane host adapters to the shared renderer');
assert(electionManagerSource.includes('test2-election-panel--main-parity') && electionManagerSource.includes('renderMainParityPartyTable') && electionManagerSource.includes('Candidates') && electionManagerSource.includes('1st preferences'), '/test2 visible election pane must follow the main grouped party-table contract');
