...
assert(electionPaneContractSource.includes('test2ElectionCountDetail') && electionPaneContractSource.includes('election-detail-toggle-btn--header') && electionPaneContractSource.includes('Detailed View: On'), '/test2 count tables must expose the main-style detailed count toggle in the pane header');
...
assert(electionPaneContractSource.includes('class Test2MainElectionPaneContract') && electionPaneContractSource.includes('renderHeaderRight') && electionPaneContractSource.includes('renderPanelContent'), '/test2 must expose an explicit main election pane contract for visible pane parity');
assert(electionPaneContractSource.includes('data-election-renderer="test2-main-pane-contract"'), '/test2 main election pane contract must stamp visible pane output for parity tests');
assert(electionManagerSource.includes('this.mainPaneContract = new Test2MainElectionPaneContract(this)') && electionManagerSource.includes('this.mainPaneContract.renderHeaderRight(selectedResult, nextView)') && electionManagerSource.includes('this.mainPaneContract.renderPanelContent(selectedResult, nextView)'), '/test2 visible election pane header/content must enter through the main-pane contract');
...
assert(electionPaneContractSource.includes('test2-election-panel--main-parity') && electionManagerSource.includes('renderMainParityPartyTable') && electionManagerSource.includes('Candidates') && electionManagerSource.includes('1st preferences'), '/test2 visible election pane must follow the main grouped party-table contract');
assert(electionManagerSource.includes('renderConstituencyPartyTable') && electionManagerSource.includes('election-results-table--constituency-party'), '/test2 selected constituency/DEA party panes must use a main-style party table before falling back to candidate/count panes');
...
assert(electionPaneContractSource.includes('this.host.activeBundle.mainLikePartySummary') && electionManagerSource.includes('this.previousBundle?.mainLikePartySummary'), '/test2 election pane must consume main-compatible current and previous party summaries');
