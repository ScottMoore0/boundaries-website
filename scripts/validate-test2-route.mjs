...
assert(mapControllerSource.includes('directPanGestureInstalled') && mapControllerSource.includes('directPanFrame') && mapControllerSource.includes('schedulePan(nextCenter)') && mapControllerSource.includes('directGestureActive'), '/test2 must keep a frame-coalesced direct MapLibre pan fallback so desktop and mobile drag gestures do not depend solely on the native handler path or per-move URL updates');
