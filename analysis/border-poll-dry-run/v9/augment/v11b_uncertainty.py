...
conpop=dz.groupby('con')['population'].sum()
cath21=dz.groupby('con').apply(lambda g:np.average(g['catholic_bg_pct'],weights=g['population']))
NI11=np.average(cath11.reindex(cath21.index),weights=pop11.reindex(cath21.index))     # pop-weighted NI Catholic-bg 2011
NI21=np.average(cath21,weights=conpop)                                                # 2021
...
fwd=np.array(fwd)
print(f"  NI Catholic 2011(background,KS212) {NI11:.1f}% vs 2021(dz21 measure) {NI21:.1f}%")
print("  CAVEAT: the 2021 field appears to be Catholic RELIGION (~42%), not BACKGROUND (~45.7%),")
print("          so the 2011<->2021 SIGN is a measure artifact; use |magnitude| only.")
...
print("       scenario, not a forecast. A clean run needs the 2021 community-BACKGROUND field for consistency.")
