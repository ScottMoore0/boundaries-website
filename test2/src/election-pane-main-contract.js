...
export class Test2MainElectionPaneContract {
  constructor(host) {
    this.host = host;
  }
  ...
  renderOverallResults(view = 'party') {
    ...
    return `
      <section class="test2-election-panel test2-election-panel--main-parity" aria-label="Election results summary" data-election-renderer="test2-main-pane-contract">
  ...
  renderConstituencyResults(result, view = 'party') {
    ...
    return `
      <section class="test2-election-panel test2-election-panel--main-parity" aria-label="${escapeHtml(result.constituency)} results" data-election-renderer="test2-main-pane-contract">
        <dl class="test2-election-panel__stats">
          ...
          ${result.seatsTotal ? `<div><dt>Seats</dt><dd>${this.host.formatNumberForPane(result.seatsTotal)}</dd></div>` : ''}
          ${result.validPoll ? `<div><dt>Valid poll</dt><dd>${this.host.formatNumberForPane(result.validPoll)}</dd></div>` : ''}
          ${result.turnoutPct ? `<div><dt>Turnout</dt><dd>${this.host.formatPercentForPane(result.turnoutPct)}</dd></div>` : ''}
          ${result.quota ? `<div><dt>Quota</dt><dd>${this.host.formatNumberForPane(result.quota)}</dd></div>` : ''}
          ${result.previous ? `<div><dt>Previous winner</dt><dd>${escapeHtml(result.previous.winnerParty || result.previous.leadingParty || '')}</dd></div>` : ''}
          ${result.deltas?.turnoutPct !== null && result.deltas?.turnoutPct !== undefined ? `<div><dt>Turnout change</dt><dd>${this.host.formatSignedPercentForPane(result.deltas.turnoutPct)}</dd></div>` : ''}
        </dl>
